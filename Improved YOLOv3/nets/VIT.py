...
class PatchEmbed(nn.Module):
    def __init__(self, input_shape=[416, 416], patch_size=16, in_chans=3, num_features=768, norm_layer=None,
                 flatten=True):
        super().__init__()
...
'''class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        # self.sin_attn = SingleVisionTransformer()
        #   加入单头注意力就够了
        self.sattn = Sin_Attention(dim)



    def forward(self, x):
        x1 = x + self.drop_path(self.attn(self.norm1(x)))
       # print(x1.size())   #    [2, 197, 768]
        x_sn = self.drop_path(self.sattn(self.norm1(x)))
      #  print(x_sn.size())  #
        x = x1 + x_sn
...
class VisionTransformer(nn.Module):
    def __init__(
            self, input_shape=[416, 416], patch_size=16, in_chans=3, num_classes=3, num_features=768,
            depth=2, num_heads=2, mlp_ratio=4., qkv_bias=True, drop_rate=0.1, attn_drop_rate=0.1, drop_path_rate=0.1,
        ...
        num_patches = (416 // patch_size) * (416 // patch_size)
        self.num_features = num_features
        self.new_feature_shape = [int(input_shape[0] // patch_size), int(input_shape[1] // patch_size)]
        self.old_feature_shape = [int(416 // patch_size), int(416 // patch_size)]
        ...
        self.attn = Attention(num_features + 1)
        self.sin_attn = Sin_Attention(num_features + 1)
    ...
    def forward_features(self, x):
        #   将cls_token与image_token拼接
        x = self.patch_embed(x)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_token, x), dim=1)

        #   将cls_token_pe与image_token_pe拼接
...
# model_attn = VisionTransformer([52, 52])
# device = torch.device('cuda:0')
# model_attn = model_attn.to(device)

def vit(input_shape=[416, 416], pretrained=False, num_classes=1000):
...
model_attn = vit(num_classes=1000, pretrained=False).train().cuda()
